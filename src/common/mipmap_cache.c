...
{
  char *min = dt_conf_get_string(config_name);
                    ...
                    const dt_mipmap_size_t size)
{
  ...
  if(!altered && !dt_conf_get_bool("never_use_embedded_thumb") && !incompatible)
  {
    ...
    else
    {
      ...
      if (!res)
      {
        // Check if embedded thumbnail meets the minimum required in
        // the configuration:
        res = !check_thumb_quality(thumb_width, thumb_height, "embedded_thumb_minimum");

        if (res)
        {
          dt_print(DT_DEBUG_CACHE,
                   "[_init_8] ignoring %dx%d thumbnail for mipmap\n",
